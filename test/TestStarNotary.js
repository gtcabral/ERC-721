...
it('can add the star name and star symbol properly', async() => {
    ...
    let starToken = await instance.lookUptokenIdToStarInfo(6);
    assert.equal(star, starToken);
});
...
it('lets 2 users exchange stars', async() => {
    ...
    assert.equal(await instance.ownerOf.call(starId1), user2, "error 1");
    assert.equal(await instance.ownerOf.call(starId2), user1, "error 2");
});
